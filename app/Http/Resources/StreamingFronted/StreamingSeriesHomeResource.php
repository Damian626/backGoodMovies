...
class StreamingSeriesHomeResource extends JsonResource
{
    /**
    ...
    public function toArray($request)
    {
        ...
        // ["JOSE","PEPITO"]
        $last_season = $this->resource->seasons->last();
        $episodes = collect([]);
        foreach ($last_season->episode_actives as $key => $episode) {
            $episodes->push([
                "id" => $episode->id,
                "title" => $episode->title,
