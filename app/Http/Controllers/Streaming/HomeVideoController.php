...
{
    function home() {
        $slider_home = Streaming::where("state",2)->where("type",3)->inRandomOrder()->limit(3)->get();

        $last_movies_a = Streaming::where("state",2)->where("type",3)->inRandomOrder()->limit(5)->get();
        $last_movies_b = Streaming::where("state",2)->where("type",3)->inRandomOrder()->limit(5)->get();
        $last_movies_c = Streaming::where("state",2)->where("type",3)->inRandomOrder()->limit(5)->get();
